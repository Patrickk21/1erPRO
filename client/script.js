...
let messages = [] // Déclaration de la variable messages

function displayChatHistory() {
    messages.forEach(({ sender, message }) => {
        const uniqueId = generateUniqueId()
        chatContainer.innerHTML += chatStripe(sender === 'bot', message, uniqueId)
    })
}
...
function generateUniqueId() {
    ...
    const hexadecimalString = randomNumber.toString(16).padStart(16, '0');
...
const handleSubmit = async (e) => {
    ...
    messages.push({ sender: 'bot', message: "" })
    ...
    if (response.ok) {
        ...
        messages[messages.length - 1].message = parsedData
    } else {
